...
НЕВИДИМЫЕ И НДЕКСЫ.
 Невидимые и ндексы являются невидимыми только в том смысле, что оптимизатор не рассматривает их применение 
 при генерации планов выполнения, если только он не проинструктирован о необходимости их использовать. 
 
 Итак, в чем состоит польза от невидимых индексов? Такие и ндексы все равно будут поддерживаться (отсюда и снижение 
...
Работают ли индексы в представлениях?
  Любые и ндексы, которые могли использоваться в запросе, написанном в отношении к базовым таблицам, будут учитываться 
...
Могут ли значения NULL и индексы работать вместе?
  И ндексы со структурой В-дерева, за исключением особого случая кластерных индексов со структурой В-дерева, не хранят записи,
